...
def render_brand_header() -> None:
    st.markdown(
        ...
        .nutreco-title {font-size: 2rem; font-weight: 700; margin-bottom: 0.15rem;}
        .nutreco-subtitle {font-size: 0.98rem; color: #425466; margin-bottom: 0.25rem;}
        .brand-note {font-size: 0.82rem; color: #6b7280; margin-top: 0.2rem;}
        </style>
        """,
    ...
    with c1:
        if logo_nutreco is not None:
            ...
            st.markdown("<div class='nutreco-title'>Nutreco Iberia</div>", unsafe_allow_html=True)
        st.markdown(
            "<div class='nutreco-title'>Actualización automática PPA España</div>",
            unsafe_allow_html=True,
        )
        st.markdown(
            "<div class='nutreco-subtitle'>Comité de Dirección · vigilancia epidemiológica, regionalización, medidas administrativas y mercado porcino</div>",
            unsafe_allow_html=True,
        )
    with c2:
...
def merge_note_with_history(current_note: dict[str, Any], parsed_notes: list[dict[str, Any]]) -> dict[str, Any]:
    """Mantiene siempre visibles los datos generales aunque la última nota sea solo de regionalización."""
    merged = dict(current_note)
    ...
    for candidate in parsed_notes[1:]:
        if any(candidate.get(field) is not None for field in general_fields):
            fallback_note = candidate
            break

    if fallback_note is not None:
        merged["stats_source_date"] = fallback_note.get("report_date")
        merged["using_fallback_stats"] = any(merged.get(field) is None for field in general_fields)
        for field in fallback_fields:
            current_value = merged.get(field)
            if current_value is None or current_value == []:
                fallback_value = fallback_note.get(field)
                if fallback_value is not None and fallback_value != []:
                    merged[field] = fallback_value
    else:
        merged["stats_source_date"] = merged.get("report_date")
    ...
    if merged.get("new_foci") is None and previous_comparable:
        current_total_foci = merged.get("total_foci")
        previous_total_foci = previous_comparable.get("total_foci")
        if current_total_foci is not None and previous_total_foci is not None:
            merged["new_foci"] = current_total_foci - previous_total_foci

    if merged.get("new_positives") is None and previous_comparable:
        current_total_positives = merged.get("total_positives")
        previous_total_positives = previous_comparable.get("total_positives")
        if current_total_positives is not None and previous_total_positives is not None:
            merged["new_positives"] = current_total_positives - previous_total_positives

    if fallback_note is not None and merged.get("previous_note_date") is None:
        merged["previous_note_date"] = fallback_note.get("report_date")
...
latest_note = merge_note_with_history(parsed_notes[0], parsed_notes)
previous_note = latest_note.get("previous_comparable_note")
...
with tab1:
    ...
    with left:
        ...
        if latest_note.get("using_fallback_stats"):
            bullets.append(
                f"Las cifras generales visibles proceden de la última nota con datos epidemiológicos completos ({format_date_es(latest_note.get('stats_source_date'))})."
            )
    ...
    with right:
        ...
        if latest_note.get("using_fallback_stats"):
            briefing.append(
                f"Consolidado general mantenido desde la nota del {format_date_es(latest_note.get('stats_source_date'))}"
            )
